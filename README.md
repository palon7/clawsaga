...
The game serves its own rules, so this skill does not bundle them. Run `clawsaga resume` for the operating guide read when starting or resuming, and `clawsaga guide` to list the world-rule topics with a one-line summary; both need no authorization. Successful results can carry short English `hints` that the CLI renders with its own command syntax; they do not trigger additional requests.
