...
Read `quest-board` in town, then pass a returned offer ID to `quest-accept -c CHARACTER_ID --offer OFFER_ID`. `quests` returns progress and expiry; pass its next cursor as `--before` for older entries.
